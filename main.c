...
int main()
{
    ...
    // 5 categories, 10 items each
    float price[5][11] = {
        {1.0f, 3.15f, 8.60f, 1.63f, 1.06f, 5.30f, 1.38f, 7.0f, 1.16f, 1.46f, 1.0f},
    ...
    // Variables
    float pay = 0, change = 0, total = 0.0f;
    ...
    while (passA)
    {
        ...
        total += price[ansType - 1][ansItem] * amount;
        printf("\n++++++++++++++++++++++++++++++++++++++++++++++++\n");
        shopList[listCurrent][0] = ansType - 1;
        shopList[listCurrent][1] = ansItem;
        shopList[listCurrent][2] = amount;
        listCurrent++;
        printf("Added %s to the shopping list (+$%.2f). \nYour total is now $%.2f (+$%.2f Tax)", grocery[ansType - 1][ansItem], price[ansType - 1][ansItem] * amount, total * 1.08, total * 0.08f);
        printf("\n++++++++++++++++++++++++++++++++++++++++++++++++\n");
        ...
        {
            printf("[%d] %s ($%.2f)  x%d\n", i, grocery[shopList[i - 1][0]][shopList[i - 1][1]], price[shopList[i - 1][0]][shopList[i - 1][1]] * amount, amount);
        }
    ...
    while (passp)
    {
        printf("\nYour total is $%.2f (+$%.2f Tax).", total * 1.08, total * 0.08f);
        printf("\nYour money : ");
        scanf("%f", &pay);
        if (pay < total)
        {
